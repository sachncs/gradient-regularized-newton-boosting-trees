...
MultiClassNewtonTree extends this to vector-valued outputs (K classes),
where leaf weights are K-dimensional vectors and split gain sums across classes.
"""
...
class _MultiClassNode:
    """Internal node for multi-class decision tree."""
...
class MultiClassNewtonTree:
    ...
    Split quality is measured by the sum of Newton gains across all K classes.
    Each leaf stores a K-dimensional weight vector.
    ...
    def _predict_one(self, node: _MultiClassNode, sample: np.ndarray) -> np.ndarray:
        """Traverse tree for a single sample, returning K-dim weight."""
        if node.is_leaf:
    ...
    def _build(
        ...
        # Leaf weight: w_k = -sum(g_k) / (sum(h_k) + lam) for each class k
        ...
        for feat in range(x.shape[1]):
            ...
            for j in range(x_sorted.shape[0] - 1):
                ...
                gr = g_total - gl  # (K,)
                hr = h_total - hl  # (K,)
                # Sum Newton gain across all classes
                gain = float(
                    np.sum(
                        0.5
                        * (
